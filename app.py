...
if uploaded_file is not None:
    try:
        ...
        if error_msg:
            ...
            result_df = result_df.sort_values(by=['신학년반', '성별_order', '이름']).reset_index(drop=True)
            ...
            def show_class_table(class_name):
                # 해당 반 데이터 추출
                subset = result_df[result_df['신학년반'] == class_name][display_cols].reset_index(drop=True)
